...
View an agent's runs from its detail page or the **Runs** tab. Each run shows the complete log and, if available, a formatted result card. You can mark a run's outcome from its detail page; trading agents with `reviewDecisions` will see these outcomes on their next run via `get_my_decisions`.
