...
The Telegram gateway runs in-process using long polling — no webhook, no public URL required. Config is colocated with the agent at `adapterConfig.gateways.telegram`.

**Pairing flow (on `/control-tower`):**

1. Click the Telegram tile → paste a bot token from [@BotFather](https://t.me/BotFather).
